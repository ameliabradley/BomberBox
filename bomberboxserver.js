...
function start_gameserver(maps, options, shared) {
  ...
  */
  ...
  function setupWorldServer () {
    worldServer = new WorldServer(broadcast, log);
  ...
  server.on("connection", function (conn) {
    ...
    */
    conn.post = function (iCommandId, o) {
      var strPacket = msgpack.pack([iCommandId, o], true);
      conn.send(strPacket);
    }
    ...
    */
    conn.set_state = function (new_state) {
      switch (new_state) {
        ...
        case STATE_JOINED:
          worldConnection = worldServer.createConnection(conn.ip, conn.post);
          break;
    ...
    */
    ...
    conn.on('message', function (data) {
      ...
      switch (packet[0]) {
        ...
        default:
        worldConnection.handleClientRequest(packet);
        break;
      }
    });
